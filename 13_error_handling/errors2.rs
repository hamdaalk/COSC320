...
// ? Works only in functions that return Result<T, E> or Option<T>.
//The ? operator in Rust automatically propagates errors by returning early if an operation fails, simplifying error handling.
//if item_quantity.parse::<i32>() fails, ? immediately returns the ParseIntError instead of continuing execution.
